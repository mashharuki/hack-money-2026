...
## L2 Admin：計算資源の中央銀行モデル

Ghost Yield において、**L2オペレーターは「Compute Central Bank（計算資源の中央銀行）」** として機能します。CPTという"計算通貨"の発行者であり、その価値はL2の実在するインフラ能力に裏付けられています。

中央銀行が通貨供給量・金利・公開市場操作で金融政策を運営するように、L2 Adminは**3つの連動するレバー**でCPT経済を管理します。そしてこれらはすべて、**L2稼働率（Utilization Rate）** という単一のシグナルから自動決定されます。

```mermaid
flowchart TD
  subgraph Oracle["Oracle Layer（単一シグナル）"]
    BOT["Bot（EMA、毎分更新）"]
    CL["Chainlink Functions（15分毎に検証）"]
    ORACLE["L2 Utilization Oracle"]
    BOT -->|高速更新| ORACLE
    CL -->|安全な検証| ORACLE
  end

  subgraph Admin["L2 Admin — 3つのレバー"]
    MINT["レバー1: CPT供給量\n（キャパシティ裏付けミント）"]
    LIQ["レバー2: 流動性の深さ\n（余剰比例）"]
    FEE["レバー3: 動的手数料\n（稼働率スケール）"]
  end
  ...
  MINT -->|"供給量 = capacity × (1 − u)"| POOL
  LIQ -->|"深さ = base × (1 − u)²"| POOL
  FEE -->|"手数料 = min + (max − min) × u²"| POOL
```

### レバー1：CPT供給量 — キャパシティ裏付けミント

CPTの発行量は恣意的なものではなく、**L2チェーンの実在する計算能力**に上限が設定されます。
...
BlocksPerEpoch = 300 blocks（約1時間）
GSU_PER_CPT    = 1,000,000

MaxMintPerEpoch = (30M × 300) / 1M = 9,000 CPT

稼働率 10% → 最大 8,100 CPT をミント可能（余剰90%）
稼働率 50% → 最大 4,500 CPT をミント可能（余剰50%）
稼働率 90% → 最大   900 CPT をミント可能（余剰10%）
```

**意味：** オペレーターは**実際に空いている計算能力の分だけ**CPTをミントできます。これにより供給インフレを防止し、すべてのCPTが「消費可能な実在のブロックスペース」に裏付けられることが保証されます。

### レバー2：流動性供給 — 余剰キャパシティ連動の深さ

L2オペレーターは、余剰計算能力に比例してUniswap v4のCPT/USDCプールに流動性を提供します。
...
| 稼働率 | 倍率 | 市場への影響 |
|:------:|:----:|:-----------|
| 10% | × 0.81 | 分厚い流動性 → スリッページ小 → CPTを買いやすい |
| 30% | × 0.49 | 中程度の深さ → 通常の取引環境 |
| 50% | × 0.25 | 薄い流動性 → 価格インパクトが出始める |
| 70% | × 0.09 | 浅い → 大口取引で価格が大きく動く |
| 90% | × 0.01 | 極薄 → CPTが希少かつ高価になる |

**なぜ二乗（²）か？** 線形関数では流動性の除去が緩やかすぎます。二乗項を使うことで**非線形の希少性カーブ**が生まれます。低稼働時はプールが豊富な流動性を提供し（トレーダーを誘引）、高稼働時は流動性が急速に消滅します（実際の資源希少性を反映）。これは現実のコモディティ市場と同じ挙動です — 余っている商品は安くて潤沢、希少な商品は板が薄い。

### レバー3：動的手数料 — 稼働率に連動する連続関数

Uniswap v4 Hook は、離散的なティアではなく**稼働率に応じて滑らかにスケールする連続手数料関数**を適用します。
...
パラメータ:
  u      = 稼働率（0.0 〜 1.0）
  FeeMin = 0.05%（500 bps）— 空いているチェーンの最小摩擦
  FeeMax = 1.00%（10000 bps）— 混雑チェーンの最大摩擦
```

| 稼働率 | 手数料 | 解釈 |
|:------:|:-----:|:----|
| 10% | 0.06% | ほぼ無料 — 空いているL2での取引を促進 |
| 30% | 0.14% | 低コスト — アービトラージャーを惹きつける |
| 50% | 0.29% | 中程度 — バランスの取れた市場 |
| 70% | 0.52% | やや高い — 過度な消費を抑制 |
| 90% | 0.82% | 高い — 希少なリソースを保護 |

**なぜ二乗（u²）か？** この設計は**TCP輻輳制御**と同じ発想です：
- 低稼働帯では手数料がほぼフラット → 取引量とアービトラージ機会を最大化
- 高稼働帯では手数料が急勾配で上昇 → 需要を自然に抑制し、L2の過剰コミットを防止

### 3つのレバーの統合メカニズム

3つのレバーは**自己強化型の市場メカニズム**を形成します：
...
L2が空いている場合（低稼働率）:
  ...
  → 流動性が深い（取引しやすい）
  → 手数料が安い（買いやすい）
  → 結果: CPTが安い → 買い手が集まる → オペレーターに収益

L2が混雑している場合（高稼働率）:
  → CPTのミントが制限される（供給制約）
  → 流動性が薄い（価格インパクト大）
  → 手数料が高い（取引コスト増）
  → 結果: CPTが高い → 実際の希少性を反映 → 単位あたり収益が増加
```

> **どちらの場合でもL2オペレーターは収益を得られます。このメカニズムは「空いている計算資源を収益化」しつつ「混雑した計算資源にはプレミアムを付ける」ことを保証します。市場は常に機能します。**

### ピッチ用の一文サマリー

> **「L2 Adminは"Compute Central Bank"として機能する：稼働率というたった1つのシグナルから、CPTの供給量・流動性の深さ・取引手数料の3つが連動して自動調整され、空いたブロックスペースを収益を生む資産に変える」**
...
**Bot (Primary Path)** と **Chainlink Functions (Verification Path)** のハイブリッド構成により、速報性と信頼性を両立しています。
...
### 2-3. CPT/USDC 基準市場（Uniswap v4 Pool）でのSwap
...
### 2-4. Uniswap v4 Hook（稼働率連動の手数料/スプレッド調整）
...
### 2-6. Ghost Arbitrage Engine（戦略生成 → Yellowセッション開始）
...
### 2-8. 最終決済（Arc + USDC）→ Operator Vault入金
...
### 2-9. Dashboard（可視化：価格・Hook状態・セッションログ・Vault残高）
...
| Hybrid Oracle   | Chainlink Functions | Bot(EMA)とChainlinkによる稼働率の二重検証 |
...
| Oracle Bot              | Node.js / TypeScript          | L2稼働率(EMA)を定期的にOracleへ反映      |
...
### 2-8. 最終決済（Arc + USDC → Operator Vault）
...
### 2-9. Yellow SDK 統合（ClearNode Sandbox）
